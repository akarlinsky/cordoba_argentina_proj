...
Create a new dataframe that contains the observed number of deaths in 
...
Compute Pearson Correlation Coefficient between projected number of deaths and registered:
```{r}
corr_proj_deaths <- acm_proj %>% 
  filter(between(year, 2019, 2020)) %>% 
  summarise(r = cor(deaths_proj_acm, deaths_arg)) %>% 
  as.numeric()
...
plot_covid_province <- covid_by_province %>% 
  ...
  #              date_labels = "%m-%y") +
    scale_x_date(breaks = seq(as.Date("2020-03-01"), as.Date("2021-12-31"), by="2 months"), 
                labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                             paste(month(x, label = TRUE), "\n", year(x)), 
                                             paste(month(x, label = TRUE))),
                expand = expansion(0.01)) +
  labs(#title = "COVID Deaths in Argentina by Province",
       y = "Daily COVID-19 deaths (7-day moving average)",
       x = "Day") +
